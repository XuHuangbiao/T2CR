...
def get_args():
    ...
    parser.add_argument('--benchmark', type = str, choices=['FineDiving', 'MTL', 'Seven'], help = 'dataset')
    parser.add_argument('--prefix', type = str, default='default', help = 'experiment name')
    parser.add_argument('--resume', action='store_true', default=False ,help = 'resume training (interrupted by '
                                                                               'accident)') 
...
def setup(args):
    ...
    args.config = 'configs/{}_T2CR.yaml'.format(args.benchmark)
    args.experiment_path = os.path.join('./experiments','T2CR', args.benchmark, args.prefix)
    if args.resume:
