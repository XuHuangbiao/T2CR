...
def train_net(args):
    ...
    # build model
    base_model, taa, regressor, decoder = builder.model_builder(args)
    ...
    optimizer, scheduler = builder.build_opti_sche(base_model, taa, regressor, decoder, args)
    ...
    if args.resume:
        start_epoch, epoch_best_aqa, rho_best, L2_min, RL2_min = \
            builder.resume_train(base_model, taa, regressor, decoder, optimizer, args)
        print('resume ckpts @ %d epoch(rho = %.4f, L2 = %.4f , RL2 = %.4f)'
              % (start_epoch - 1, rho_best, L2_min, RL2_min))

    # DP
    base_model = nn.DataParallel(base_model)
    taa = nn.DataParallel(taa)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for idx, (data, target) in enumerate(train_dataloader):
            ...
            if args.benchmark == 'MTL':
                ...
                class_1 = data['dive_class'].float().cuda()
                class_2 = target['dive_class'].float().cuda()
                class_1 = class_1.reshape(class_1.shape[0],-1)
                class_2 = class_2.reshape(class_2.shape[0], -1)
            ...
            # forward
            helper.network_forward_train(base_model, taa, regressor, decoder, pred_scores,
                                         video_1, label_1, class_1, video_2, label_2, class_2, diff, mse, bce, optimizer,
                                         opti_flag, epoch, idx + 1, len(train_dataloader), args)
        ...
        validate(base_model, taa, regressor, decoder, test_dataloader, epoch, optimizer, args)
        helper.save_checkpoint(base_model, taa, regressor, decoder, optimizer, epoch, epoch_best_aqa, rho_best, L2_min, RL2_min,
                               'last',args)
...
def validate(base_model, taa, regressor, decoder, test_dataloader, epoch, optimizer, args):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(test_dataloader, 0):
            ...
            helper.network_forward_test(base_model, taa, regressor, decoder, pred_scores,
        ...
        if rho > rho_best:
            ...
            helper.save_checkpoint(base_model, taa, regressor, decoder, optimizer, epoch, epoch_best_aqa,
...
def test_net(args):
    ...
    # load checkpoints
    builder.load_model(base_model, taa, regressor, decoder, args)
    ...
    test(base_model, taa, regressor, decoder, test_dataloader, args)


def test(base_model, taa, regressor, decoder, test_dataloader, args):
    global use_gpu
    global epoch_best_aqa, rho_best, L2_min, RL2_min
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(test_dataloader, 0):
            ...
            if args.benchmark == 'MTL':
                ...
                raise NotImplementedError()
            label_1 = data['final_score'].numpy()
            r_label_1 = data['completeness'].numpy()
            helper.network_forward_test(base_model, taa, regressor, decoder, pred_scores, video_1, video_2_list, label_2_list, diff,
                                        label_1, r_label_1, args)
