...
class FineDiving_Pair_Dataset(torch.utils.data.Dataset):
    ...
    def preprocess_class(self):
        ...
        for item in self.data_anno:
            dive_label.append(self.data_anno.get(item)[0])
        dive_label = list(set(dive_label))
        # print(dive_label)
        # print((len(dive_label)))
    ...
    def __getitem__(self, index):
        ...
        sample_label_encoded = self.label_encoder.transform([data['number']])
        ...
        if self.subset == 'train':
            ...
            sample_label_encoded = self.label_encoder.transform([target['number']])
            ...
            for item in choosen_sample_list:
                ...
                sample_label_encoded = self.label_encoder.transform([tmp['number']])
